...
## Unified Area / Restaurant tag toggles — 2026-09-12 (supersedes selected × groups)

- Baseline fetched origin/main 6729bb4; isolated canonical source travel-app/prototype/tag-toggle-ux. Other dirty worktrees preserved. No backfill, data migration, taxonomy, classifier, boundary, AI or API changes.
- The old × toggled a hidden checkbox/place association, never deleted a global tag. Both editors now have one fixed-order available option set with selection on the chip itself; no ×, check icon or separate selected row. Restaurant labels wrap native checkboxes; area buttons expose aria-pressed. The legacy data-area-tags-selected hook names the single combined area options row for selector compatibility, not a selected-only group.
- Shared chip metrics use 400 weight, 13px font, 20px line-height, 4px/12px padding, 1px border and 44px minimum height. Selection changes colors only; nodes never move or rebuild on toggle. Area options reconcile new/obsolete evidence nodes individually; custom add appends only a new node. Restaurant add/open/cancel overlays controls in one independent 44px grid row; area input has its own controlled-width 44px row. Empty area options display:none, no reserved row, padding or gap contribution; obsolete address row removed. Assets 20260912.1.
- Available area values (seed/custom plus current address/containment and persisted Trip vocabulary) remain separate from session.areaTags. Restaurant available labels are independent of checked working inputs/FormData. Save reads only selected values; Cancel discards edits. Stable identity/Map.has initialization and valid manual [] are unchanged. Candidate Save never writes state.places; only batch add does. Suggestions and new custom values become formal vocabulary only if finally selected on a persisted place. Vocabulary still derives solely from current Trip persisted arrays, with no defaults/registry; clearing A leaves a shared label used by B, clearing B removes its last usage.
- Local Edge mobile integration (393x852), scripts/verify-candidate-tags.cjs: real Maps intake button -> analyze/enrich/candidateDraft (hot_pot) -> editor click/bind/toggle/custom 麻辣鍋 -> Save -> committed candidate list/detail -> batch submit -> persisted place/shared payload/vocabulary only 麻辣鍋. Abandoned committed custom restaurant + area draft values leave zero vocabulary/persist calls. Real switchTrip/loadSharedTrip/applySharedTrip/render verifies empty Trip B then A. Two fixture persisted places A/B are edited through real detail/top Edit/native input/Save; vocabulary stays after A clears and disappears after B clears. Candidate and persisted Cancel, manual clear/reopen, original identity, list/detail ordering all retained.
- Browser layout checks compare every chip x/y/width/height/font/padding/border before/after toggles and retain node identity; four same-row restaurant chips do not wrap on selection. Custom focus/fill/cancel preserves restaurant row dimensions and chip positions; area input focus/fill preserves dimensions. Empty area production editor row has zero height/display:none and exactly the intended 10px header-to-input gap. New chip append preserves existing restaurant nodes/positions. Zero page errors. Fixtures provide HTTP responses and persistence spy, not UI handlers; server network/storage and external Google are not live-tested. Existing VM tests remain helper/form-adapter coverage for classification, normalization, identity, address evidence and save gates; API tests cover their mocked request boundaries. Shared A/B fixtures use a nonempty unstructured address to satisfy unchanged native required-address validation.
- Validation: targeted 261/262 (14 relevant files), full regression once 448/449. Sole failure is the known Windows CRLF source-slicing test at tests/area-tags.test.mjs:134; that file, .gitattributes, core.autocrlf and line-ending settings untouched. Browser integration passed; app/browser/all 12 API syntax and git diff --check passed. API functions exactly 12. Production App not opened.
