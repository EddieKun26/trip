...
const state = {
  ...
  // Place Pool (行程規劃 workspace) filters are client-only view state: never persisted, stored or sent.
...
// Normal maintenance migration terminals (success, no-op, marker recovery, read-only, gated
// abort) are a background concern and never surface to the user; diagnosticLines() above this
// call still logs every terminal to the console for engineering visibility.
...
function mapPlacePreviewMarkup(place) {
  ...
  const googlePhoto = identitySafePhotos(place)[0];
  const photo = place.customPhotoDataUrl
    ...
    : googlePhoto
      ? `<img src="/api/place-photo?name=${encodeURIComponent(googlePhoto.name)}" alt="${escapeHtml(place.name)}照片" loading="eager" />`
...
function updateMapPlacePreview(place) {
  ...
  state.selectedMapPlaceKey = place ? placeDetailKey(place) : "";
...
function selectMapPlace(place) {
  ...
  ensurePlaceDetails(place).then(() => {
    if (isSelectedMapDetailPlace(place)) updateMapPlacePreview(place);
  });
}
...
function shoppingCurrencyOptions(selected = "", allowUnknown = false) {
  const active = shoppingCurrencies.includes(String(selected).toUpperCase()) ? String(selected).toUpperCase() : allowUnknown ? "" : defaultShoppingCurrency();
  return (allowUnknown ? `<option value="" ${!active ? "selected" : ""}>未確認</option>` : "") + shoppingCurrencies.map((currency) => `<option value="${currency}" ${currency === active ? "selected" : ""}>${currency}</option>`).join("");
}
...
function syncPendingShoppingImportEdits(form) {
  ...
  form.querySelectorAll("[data-shopping-import-row]").forEach((row) => {
    ...
    if (!entry) return;
    if (!row.querySelector("[data-import-name]")) return;
    entry.details = {
      ...
      currency: String(row.querySelector("[data-import-currency]")?.value ?? entry.details?.currency ?? "").toUpperCase(),
...
function renderShoppingImportRows(form) {
  ...
  host.innerHTML = pendingShoppingImports.map((entry, index) => `
    <article class="shopping-import-result" data-shopping-import-row="${escapeHtml(entry.id)}">
      <div class="shopping-import-result-head"><span>商品 ${index + 1}</span><button type="button" data-remove-shopping-import="${escapeHtml(entry.id)}" ${busy || entry.recognitionBusy ? "disabled" : ""}>移除</button></div>
      ${shoppingImportProgressMarkup(entry, index)}
      ${shoppingSourceLinkMarkup(entry)}
      ${entry.needsScreenshot ? `<label class="shopping-fallback-upload">上傳截圖辨識<input type="file" accept="image/*" data-shopping-fallback="${escapeHtml(entry.id)}" ${entry.recognitionBusy ? "disabled" : ""} /></label>` : ""}
      ...
      <fieldset class="shopping-import-fields" ${entry.recognitionBusy ? "disabled" : ""}>
        ...
        <div class="field"><label>參考價格</label><input data-import-price inputmode="decimal" maxlength="14" value="${entry.details?.price ? escapeHtml(entry.details.price) : ""}" placeholder="無法確認價格時留空" /></div>
        <div class="field"><label>幣別</label><select data-import-currency>${shoppingCurrencyOptions(entry.details?.currency, Boolean(entry.sourceReferenceUrl))}</select></div>
  ...
  if (confirm) confirm.disabled = busy || pendingShoppingImports.some((entry) => entry.recognitionBusy) || !pendingShoppingImports.length;
}

function shoppingSourceLinkMarkup(item) {
  try {
    const url = new URL(item?.sourceReferenceUrl);
    if (url.protocol !== "https:" || url.username || url.password) return "";
    return `<p class="shopping-source-reference"><a href="${escapeHtml(url.href)}" target="_blank" rel="noopener noreferrer">${item.sourceType === "threads" ? "查看原 Threads 貼文" : "查看原商品連結"}</a></p>`;
  } catch { return ""; }
}

function shoppingEntryGuard(form, entry) {
  const version = (entry.requestVersion || 0) + 1;
  entry.requestVersion = version;
  const context = tripContextVersion, tripId = state.tripId, memberId = currentMemberId();
  return () => form.isConnected && entry.requestVersion === version && pendingShoppingImports.includes(entry)
    && tripContextVersion === context && state.tripId === tripId && currentMemberId() === memberId;
}

function applyShoppingRecognition(entry, result) {
  entry.details = result.details;
  entry.recognition = { language: result.source?.language || "多語言", confidence: Number(result.confidence) || 0 };
  entry.annotation = result.annotation ? { ...result.annotation, productImages: [] } : null;
  ...
  entry.needsScreenshot = false;
  entry.recognized = true;
  entry.recognitionStage = "complete";
}

async function readShoppingLink(button) {
  const form = button.closest("#shopping-import-form");
  if (!form || button.disabled || !canManageShopping()) return;
  let url;
  try {
    url = new URL(form.querySelector("[data-shopping-source-url]").value.trim());
    if (url.protocol !== "https:" || url.username || url.password) throw new Error();
  } catch { return setShoppingRecognitionStatus(form, "請貼上完整 HTTPS 連結。", { tone: "error" }); }
  if (pendingShoppingImports.length >= SHOPPING_IMPORT_MAX_FILES) return showToast("請先確認或移除待加入商品。");
  syncPendingShoppingImportEdits(form);
  const sourceType = ["threads.com", "www.threads.com", "threads.net", "www.threads.net"].includes(url.hostname) ? "threads" : "product_url";
  const entry = { id: `shopping-import-${crypto.randomUUID()}`, sourceReferenceUrl: form.querySelector("[data-shopping-source-url]").value.trim(), sourceType,
    dataUrl: "", details: { brand: "", name: "", benefits: "", price: 0, currency: "", categoryId: "daily" }, productImages: [], recognitionBusy: true, recognitionStage: "recognizing" };
  pendingShoppingImports.push(entry);
  const current = shoppingEntryGuard(form, entry);
  button.disabled = true;
  renderShoppingImportRows(form);
  setShoppingRecognitionStatus(form, sourceType === "threads" ? "正在讀取 Threads 圖文並辨識主要商品…" : "正在讀取商品資料…");
  try {
    const response = await fetch("/api/shopping-recognize", { method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ action: sourceType === "threads" ? "social-source" : "url-draft", tripId: state.tripId, sourceUrl: entry.sourceReferenceUrl, recognize: sourceType === "threads", previewImage: true }) });
    const result = await response.json().catch(() => ({}));
    if (!current()) return;
    if (!response.ok) throw new Error(result.error || "PRODUCT_NOT_RECOGNIZED");
    if (sourceType === "threads") {
      if (result.requiresScreenshot || !result.details?.name) throw new Error("SOURCE_SCREENSHOT_REQUIRED");
      if (result.sourceImageDataUrl) {
        const compressed = await fetch(result.sourceImageDataUrl).then((response) => response.blob())
          .then(compressShoppingScreenshot).catch(() => "");
        if (!current()) return;
        entry.dataUrl = compressed;
      }
      applyShoppingRecognition(entry, result);
    } else {
      const draft = result.draft;
      if (!draft?.name || !["success", "partial"].includes(draft.readStatus)) throw new Error("SOURCE_SCREENSHOT_REQUIRED");
      applyShoppingRecognition(entry, { details: { brand: draft.brand || "", name: draft.name, benefits: "", price: draft.price || 0, currency: draft.currency || "", categoryId: "daily" },
        annotation: { summary: "", productImages: draft.imageDataUrl ? [{ id: entry.id + "-source", url: draft.imageDataUrl, kind: "web-product", sourceTitle: "商品頁圖片" }] : [] } });
    }
    setShoppingRecognitionStatus(form, "已取得商品資料，請確認、修改後加入。", { progress: 1, tone: "success" });
  } catch (error) {
    if (!current()) return;
    entry.needsScreenshot = true;
    entry.recognitionStage = "error";
    entry.recognitionError = ["SOURCE_SCREENSHOT_REQUIRED", "PRODUCT_NOT_RECOGNIZED"].includes(error.message)
      ? "無法確認主要商品。原連結已保留，請上傳截圖辨識或填寫商品資料。" : shoppingRecognitionErrorMessage(error);
    setShoppingRecognitionStatus(form, "原連結已保留，可在下方上傳截圖辨識。", { tone: "error" });
  } finally {
    if (current()) { entry.recognitionBusy = false; button.disabled = false; renderShoppingImportRows(form); }
  }
}

async function handleShoppingFallbackFile(input) {
  const form = input.closest("#shopping-import-form");
  const entry = pendingShoppingImports.find((candidate) => candidate.id === input.dataset.shoppingFallback);
  const file = input.files?.[0];
  if (!form || !entry || !file || entry.recognitionBusy || !canManageShopping()) return;
  if (!file.type.startsWith("image/") || file.size > 15 * 1024 * 1024) return showToast("請選擇小於 15 MB 的截圖。");
  syncPendingShoppingImportEdits(form);
  const current = shoppingEntryGuard(form, entry);
  entry.recognitionBusy = true; entry.recognitionStage = "preparing";
  renderShoppingImportRows(form);
  try {
    const dataUrl = await compressShoppingScreenshot(file);
    if (!current()) return;
    entry.dataUrl = dataUrl; entry.recognitionStage = "recognizing";
    renderShoppingImportRows(form);
    const result = await recognizeShoppingScreenshotWithAi(entry);
    if (!current()) return;
    applyShoppingRecognition(entry, result);
    setShoppingRecognitionStatus(form, "截圖辨識完成，原連結已保留。請確認後加入。", { progress: 1 });
  } catch (error) {
    if (!current()) return;
    entry.recognitionStage = "error"; entry.recognitionError = shoppingRecognitionErrorMessage(error); entry.needsScreenshot = true;
  } finally {
    if (current()) { entry.recognitionBusy = false; renderShoppingImportRows(form); }
  }
}
...
async function recognizeShoppingScreenshots(form) {
  ...
  const recognitionContext = tripContextVersion;
  const recognitionTrip = state.tripId;
  const recognitionMember = currentMemberId();
  const current = () => form.isConnected && token === shoppingRecognitionToken && tripContextVersion === recognitionContext && state.tripId === recognitionTrip && currentMemberId() === recognitionMember;
  const confirm = form.querySelector('button[type="submit"]');
  ...
  try {
    ...
    await Promise.all(entries.map(async (entry) => {
      try {
        const result = await recognizeShoppingScreenshotWithAi(entry);
        if (!current() || !pendingShoppingImports.includes(entry)) return;
        syncPendingShoppingImportEdits(form);
        applyShoppingRecognition(entry, result);
      } catch (error) {
        if (!current() || !pendingShoppingImports.includes(entry)) return;
...
async function handleShoppingScreenshotFile(input) {
  ...
  const context = tripContextVersion, tripId = state.tripId, memberId = currentMemberId();
  const current = () => form.isConnected && pendingShoppingImports === entries && tripContextVersion === context && state.tripId === tripId && currentMemberId() === memberId;
  const confirm = form.querySelector('button[type="submit"]');
  if (confirm) confirm.disabled = true;
...
function openShoppingImportSheet(mode = "screenshot") {
  ...
  sheetRoot.innerHTML = `
    <div class="modal-backdrop shopping-backdrop" data-dismiss-sheet>
      <form id="shopping-import-form" class="modal-sheet shopping-form-sheet shopping-import-sheet" data-shopping-sheet>
        <div class="section-row shopping-sheet-header"><div><p class="section-kicker">SHOPPING IMPORT</p><h2>${mode === "link" ? "貼連結辨識" : "辨識推薦截圖"}</h2></div><div class="header-actions">${shoppingUndoButtonMarkup("sheet-undo-button")}<button class="icon-button" type="button" data-close-sheet>×</button></div></div>
        <div class="shopping-sheet-body">
          ${mode === "link" ? `<div class="field shopping-link-input"><label for="shopping-source-url">Threads 或商品連結</label><input id="shopping-source-url" type="url" data-shopping-source-url placeholder="https://…" maxlength="4096" /><button type="button" data-read-shopping-link>讀取並辨識</button><small>一次辨識一個主要商品；無法讀取時可上傳截圖。</small></div>` : ""}
          <div ${mode === "link" ? "hidden" : ""}>
...
function render({ preserveScroll = false, filterOnly = false } = {}) {
  ...
  // (a selected card leaves the candidate list entirely), so a naive scrollTop restore would jump.
...
function resolveDetailPlace(identity) {
  const exact = state.places.filter((place) => placeDetailKey(place) === identity);
  if (exact.length) return exact.length === 1 ? exact[0] : null;
  // Legacy itinerary/name links are safe only when the name is unambiguous.
  const named = state.places.filter((place) => place.name === identity);
  return named.length === 1 ? named[0] : null;
}

function isSelectedMapDetailPlace(place) {
  if (!state.selectedMapPlace || place.name !== state.selectedMapPlace) return false;
  return state.selectedMapPlaceKey
    ? placeDetailKey(place) === state.selectedMapPlaceKey
    : resolveDetailPlace(state.selectedMapPlace) === place;
}

function detailGooglePlaceId(place) {
  const stored = String(place?.placeId || "").trim();
  if (stored) return /^(?:osm-|coordinate-|manual-address-|custom-place-)/u.test(stored) ? "" : stored;
  if (!isGoogleMapsUrl(place?.sourceUrl)) return "";
  try {
    const url = new URL(place.sourceUrl);
    return url.searchParams.get("query_place_id") || url.searchParams.get("place_id") || "";
  } catch { return ""; }
}

function isAddressDetailPlace(place) {
  return Boolean(place?.manualLocation || place?.coordinateLocation || place?.addressProvider
    || /^(?:osm-|coordinate-|manual-address-|custom-place-)/u.test(String(place?.placeId || ""))
    || /^(?:建築物|建物|地址|街道地址|郵遞區號|住宿座標|premise|street_address|postal_code|route|subpremise|building)$/iu.test(String(place?.category || "").trim()));
}

function identitySafePhotos(place) {
  const id = detailGooglePlaceId(place);
...
// Hours-only enrichment owns no address/photo/detail fields. A trusted current Google
// identity remains eligible even when full Detail refresh is locked or address-based.
// Keep identity validation aligned with the authenticated hours-only server action.
function plannerHoursGooglePlaceId(place) {
  const id = detailGooglePlaceId(place);
  return /^[A-Za-z0-9_-]{1,180}$/.test(id) ? id : "";
}

function hydratePlannerPlaceHours(entry, requireWindows = false) {
  const placeId = plannerHoursGooglePlaceId(entry?.place);
...
async function ensurePlaceDetails(place) {
  ...
  try {
    const response = await fetch("/api/places", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ places: [{ sourceUrl: place.sourceUrl, placeId, resolveDetails: true }] }),
    });
    if (!response.ok) return;
    ...
    Object.assign(place, {
      ...
      photos: identitySafePhotos({ placeId, photos: resolved.photos || place.photos || [] }),
      photosLoaded: true,
    });
    applyPlanningRegionResolution(place, resolved);
    persist({ recordUndo: false });
    if (document.querySelector(`[data-detail-place="${CSS.escape(detailKey)}"]`)) openPlaceSheet(placeDetailKey(place));
  } catch {
...
// allEntries lets placePoolViewModel split the Selected column/drawer out of the unfiltered pool,
// so it is never narrowed by these filters at all (selection is a hard commitment, not something
// filters may hide); matchesFilters/entries only ever drive the candidate column.
...
function placePoolPlanningConstraints() {
  if (placePoolPlanning.tripId !== state.tripId) {
    placePoolPlanning.tripId = state.tripId;
    placePoolPlanning.constraints = new Map();
  }
  return placePoolPlanning.constraints;
}
...
// placePoolSelection/placePoolPlanning above (never part of `state.placePool`, which is not
// itself trip-scoped) so it resets to closed on a trip switch without needing clearTripView().
const placePoolDrawer = { tripId: "", open: false };

function placePoolDrawerIsOpen() {
  if (placePoolDrawer.tripId !== state.tripId) {
    placePoolDrawer.tripId = state.tripId;
    placePoolDrawer.open = false;
  }
  return placePoolDrawer.open;
}

function setPlacePoolDrawerOpen(open) {
  placePoolDrawer.tripId = state.tripId;
  placePoolDrawer.open = Boolean(open);
  render({ preserveScroll: true, filterOnly: true });
}
...
// pruned here rather than counted, and its constraint is dropped with it: the selected count
// always reflects currently valid entries, never the raw key Set size.
function placePoolSelectedEntries(pool = getUnscheduledPlaces()) {
  ...
  for (const key of [...keys]) if (!validKeys.has(key)) { keys.delete(key); clearPlacePoolConstraint(key); }
...
function plannerCandidateHoursMarkup(entry) {
  const conflict = placePoolHoursConflicts.get(entry.key);
  if (conflict) return plannerHoursWarningMarkup(conflict);
  const id = plannerHoursGooglePlaceId(entry.place);
  if (!id || !placePoolConstraintFor(entry.key).some(option => option.mode === "exact")) return "";
  const unavailable = structuredHoursFetched(entry.place, id) && entry.place.regularOpeningPeriods.status === "unavailable";
  const failed = plannerHoursFailures.has(id);
  if (!unavailable && !failed) return "";
  return `<div class="place-pool-hours-warning place-pool-hours-notice" role="status"><strong>${unavailable ? "營業時間無法確認" : "營業時間暫時無法確認"}</strong><p>規劃時不會將此地點的營業時間作為硬性限制，請自行確認。</p></div>`;
}
...
function placePoolToggleMarkup(pool) {
  return `<button class="place-pool-toggle" type="button" data-toggle-place-pool aria-expanded="${state.placePool.open}" aria-controls="place-pool-panel" aria-label="行程規劃，${pool.total} 個尚未安排"><span>行程規劃</span><b>${pool.total}</b></button>`;
}
...
function placePoolCardMarkup({ key, place, voteCount }, { docked, selected }) {
  ...
  return `
          <li class="place-pool-item" data-pool-anchor-key="${escapeHtml(key)}">
            ...
            ${selected ? plannerCandidateHoursMarkup({ key, place }) : ""}
          </li>`;
}
  ...
  return `
          <li class="place-pool-selected-item" data-pool-anchor-key="${escapeHtml(key)}">
            <button class="place-pool-selected-unselect" type="button" data-pool-place="${escapeHtml(key)}" aria-pressed="true" aria-label="取消選取「${escapeHtml(place.name)}」">
              <span class="place-pool-selected-check" aria-hidden="true">✓</span>
            </button>
            ...
            ${plannerCandidateHoursMarkup({ key, place })}
          </li>`;
}

// Desktop: a static left column. Mobile: the same markup becomes the right-side Selected drawer
// via CSS only (one source of truth, no duplicated markup). Never collapses past any count —
// the column/drawer scrolls independently instead.
function placePoolSelectedColumnMarkup(pool) {
  const count = pool.selectedEntries.length;
  const list = count
    ? `<ul class="place-pool-selected-list" id="place-pool-selected-list" data-place-pool-selected-list>${pool.selectedEntries.map((entry) => placePoolSelectedCompactCardMarkup(entry)).join("")}</ul>`
    : `<div class="place-pool-selected-empty">還沒有選擇地點</div>`;
  return `
        <aside class="place-pool-selected-column" id="place-pool-selected-column" aria-label="已選地點">
          <h3 class="place-pool-selected-heading">已選 ${count} 個</h3>
          ${list}
        </aside>`;
}

// Fixed to the right-center of the mobile viewport at every selected count, including 0, so the
// drawer entry point is visible before the first selection is ever made. Chinese label/count are
// stacked as separate lines, never rotated as one block.
function placePoolMobileHandleMarkup(count) {
  const open = placePoolDrawerIsOpen();
  return `
        <button class="place-pool-mobile-handle" type="button" data-pool-drawer-toggle aria-expanded="${open}" aria-controls="place-pool-selected-column" aria-label="已選地點，${count} 個">
          <span class="place-pool-mobile-handle-check" aria-hidden="true">✓</span>
          <span class="place-pool-mobile-handle-label">已選</span>
          <b class="place-pool-mobile-handle-count">${count}</b>
          <span class="place-pool-mobile-handle-chevron" aria-hidden="true">${open ? "›" : "‹"}</span>
        </button>`;
}
...
function poolConstraintHourWheelOptions() {
  return Array.from({ length: 24 }, (_, hour) => String(hour).padStart(2, "0"))
    .map((value) => `<button class="time-wheel-option" type="button" data-pool-wheel-value="${value}">${value}</button>`).join("");
}

function poolConstraintMinuteWheelOptions() {
...
function confirmPlacePoolConstraint() {
  ...
  closeSheet();
  render({ preserveScroll: true, filterOnly: true });
  if (!ok) showToast("這個地點已不在行程規劃中");
}

/* Scroll-anchor preservation: identify a stable content anchor before a re-render (by key, its
 * pixel offset within the scroll container, and its immediate neighbor keys for a deterministic
 * fallback when that exact Place was the one selected/removed) and restore the same visual
 * position afterwards instead of a raw scrollTop, since selecting/filtering/constraint changes
 * the list's height. placePoolScrollAnchorFromRects/placePoolScrollTopFromAnchor are pure math
 * over plain { key, top } rects and are unit-tested directly; the remaining functions wire that
 * math to the live DOM and only run in a real browser. */
function placePoolScrollAnchorFromRects(rects, scrollTop) {
  if (!rects.length) return null;
  let index = 0;
  for (let i = 0; i < rects.length; i += 1) {
    if (rects[i].top <= scrollTop + 1) index = i; else break;
  }
  return { key: rects[index].key, offset: scrollTop - rects[index].top, prevKey: rects[index - 1]?.key ?? null, nextKey: rects[index + 1]?.key ?? null };
}

function placePoolScrollTopFromAnchor(rects, anchor) {
  if (!anchor || !rects.length) return null;
  const byKey = new Map(rects.map((rect) => [rect.key, rect]));
  const exact = byKey.get(anchor.key);
  if (exact) return Math.max(0, exact.top + anchor.offset);
  const fallback = byKey.get(anchor.nextKey) || byKey.get(anchor.prevKey) || rects[0];
  return Math.max(0, fallback.top);
}

function placePoolListItemRects(container) {
  if (!container?.querySelectorAll) return [];
  const containerTop = container.getBoundingClientRect().top - container.scrollTop;
  return [...container.querySelectorAll("[data-pool-anchor-key]")].map((element) => ({
    key: element.dataset.poolAnchorKey,
    top: element.getBoundingClientRect().top - containerTop,
  }));
}

function capturePlacePoolAnchor(selector) {
  const container = document.querySelector(selector);
  if (!container) return null;
  return { selector, anchor: placePoolScrollAnchorFromRects(placePoolListItemRects(container), container.scrollTop) };
}

function restorePlacePoolAnchor(captured) {
  if (!captured?.anchor) return;
  const container = document.querySelector(captured.selector);
  if (!container) return;
  const top = placePoolScrollTopFromAnchor(placePoolListItemRects(container), captured.anchor);
...
document.addEventListener("click", async (event) => {
  ...
  if (event.target.closest("[data-import-shopping-link]")) return canManageShopping() ? openShoppingImportSheet("link") : guestOnlyMessage();
  const readShoppingLinkButton = event.target.closest("[data-read-shopping-link]");
  if (readShoppingLinkButton) return readShoppingLink(readShoppingLinkButton);
  ...
  if (focusMapPlace) {
    const identity = focusMapPlace.dataset.focusMapPlace;
    const place = filteredMapPlaces().find((candidate) => placeDetailKey(candidate) === identity);
    if (!place) return;
  ...
  const poolConstraint = event.target.closest("[data-pool-constraint]");
  if (poolConstraint) return canEdit() ? openPlacePoolConstraintSheet(poolConstraint.dataset.poolConstraint) : guestOnlyMessage();

  const poolWheelValue = event.target.closest("[data-pool-wheel-value]");
  if (poolWheelValue) {
    const column = poolWheelValue.closest("[data-pool-wheel-part]");
    const options = [...column.querySelectorAll("[data-pool-wheel-value]")];
    column.scrollTo({ top: options.indexOf(poolWheelValue) * 44, behavior: "smooth" });
    return;
  }
...
document.addEventListener("scroll", (event) => {
  ...
  const poolColumn = event.target.closest?.("[data-pool-wheel-part]");
  if (poolColumn) updatePoolConstraintWheelColumn(poolColumn);
}, true);
...
document.addEventListener("change", async (event) => {
  ...
  if (event.target.matches("[data-shopping-fallback]")) return handleShoppingFallbackFile(event.target);
...
document.addEventListener("submit", async (event) => {
  ...
  if (event.target.id === "shopping-item-form") {
    ...
    const item = {
      ...
      ...(previous?.sourceType ? { sourceType: previous.sourceType, sourceReferenceUrl: previous.sourceReferenceUrl } : {}),
  ...
  if (event.target.id === "shopping-import-form") {
    event.preventDefault();
    if (!canManageShopping()) return guestOnlyMessage();
    if (pendingShoppingImports.some((entry) => entry.recognitionBusy)) return showToast("請等待辨識完成。");
    if (event.target.dataset.shoppingSaving === "true") return;
    const context = tripContextVersion;
    const memberId = currentMemberId();
    const tripId = state.tripId;
    const form = new FormData(event.target);
    ...
    event.target.dataset.shoppingSaving = "true";
    ...
    const additions = await Promise.all(imports.map(async (entry, index) => {
      const photoId = entry.dataUrl ? `shopping-photo-${crypto.randomUUID?.() || `${Date.now()}-${index}`}` : "";
      if (photoId) pendingPhotos[photoId] = { dataUrl: entry.dataUrl, createdAt: new Date(Date.now() + index).toISOString() };
      ...
      return {
        ...
        brand: entry.details.brand,
        name: entry.details.name,
        sourceReferenceUrl: entry.sourceReferenceUrl || "",
        sourceType: entry.sourceType || "screenshot",
        benefits: entry.details.benefits,
        price: normalizeShoppingPrice(entry.details.price),
        currency: shoppingCurrencies.includes(String(entry.details.currency || "").toUpperCase()) ? String(entry.details.currency).toUpperCase() : entry.sourceReferenceUrl ? "" : defaultShoppingCurrency(),
        categoryId,
        recipientTagIds,
        note,
        purchased: false,
    ...
    if (!event.target.isConnected || !imports.every((entry) => pendingShoppingImports.includes(entry)) || context !== tripContextVersion || memberId !== currentMemberId() || tripId !== state.tripId || !tripIsHydrated()) return;
    Object.assign(state.shopping.photos, pendingPhotos);
    ...
    render();
    const savedShopping = await saveShopping();
    if (context !== tripContextVersion || memberId !== currentMemberId() || tripId !== state.tripId) return;
