...
export default async function tripHandler(request, response) {
  try {
    ...
    if (request.method === "POST" && request.body?.action === "hydrateOpeningHours") {
      ...
      // This action owns only Google hours in the sidecar. Full Detail/address/photo
      // refresh restrictions must not disqualify an otherwise trusted current identity.
