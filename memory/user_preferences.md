...
- Japanese restaurant details should show a separate fixed-height orange `Tabelog預約` button with white text directly below, but not inside, the phone card. It must share exactly the same grid-column width as the phone card. The Tabelog URL should be attached when the restaurant is added; tapping should use Tabelog's custom scheme to open the already installed App without going through a language page or region-specific App Store, and should fall back to the corresponding website only when the App is unavailable.
