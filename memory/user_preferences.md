...
- Area recognition must work from Google address data regardless of the input language. Group places by their ward/district or municipality rather than a neighborhood—for example, a Jingumae address belongs under `澀谷區（渋谷区）`. Prefer Traditional Chinese plus the true local-language name, automatically repair legacy values from Place ID/address/coordinates/Maps links, and never persist or present `待確認區域` as the final area. A Latin-script name remains valid when Latin is the location's actual local script.
