...
- Japanese restaurant details should show an orange `Tabelog預約` button inside the lower unused portion of the phone card, matching that column's width. The Tabelog URL should be attached when the restaurant is added; tapping should directly prefer the installed Tabelog App without first showing a webpage decision screen, and should fall back to the corresponding website only when the App is unavailable.
