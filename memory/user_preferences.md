...
- Use Traditional Chinese display names while retaining the true local-language place and Travel Area names. Group headings always use `繁體中文（當地語言）`.
- Travel Area is the main area travelers use to decide which places fit on the same day. It is not an address translation and must not be forced to one administrative level. Google address components are evidence only; list grouping uses `travelAreaKey`, never `place.area`, locality, sublocality, neighborhood, ward, district, or municipality strings directly. Required results include 神宮前/神南 → `澀谷（渋谷）`, 花川戶/雷門 → `淺草（浅草）`, 銀座 → `銀座（銀座）`, 練馬區 → `練馬（練馬）`, 鎌倉市 → `鎌倉（鎌倉）`, 明洞 → `明洞（명동）`, and Brooklyn → `布魯克林（Brooklyn）`. Stable UI must never expose 神宮前、神南、花川戶、雷門 as separate groups.
- Manual Travel Area assignments take precedence. Automatic recognition is complete only after the final Travel Area result passes product rules; merely receiving API data is insufficient. Every older automatic result, including version 2 and 3 completion markers, must be recalculated at the next data version. If refresh fails, retain the previous usable category and show retry/manual-edit affordances instead of leaving `正在辨識地區` forever.
