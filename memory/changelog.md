...
- Google Maps links now adapt to the device: phones and tablets navigate in place so iOS/Android hand off to the Google Maps app, while desktop browsers open a new tab with `noopener` and leave the App page untouched (a blocked popup falls back to same-tab navigation). Verified in a real browser on both a Windows desktop user agent and an Android emulation.
...
- Added an import-sheet hint explaining that booking sites block automatic reading and that lodging links should be pasted together with the host message or booking confirmation, plus `aria-hidden` on the four decorative tab-bar glyphs. Deliberately did NOT minify assets: Vercel already serves app.js as 85.6 KB brotli from 334 KB, so a new build pipeline was not justified.
