...
- Corrected area grouping granularity from neighborhood to ward/district or municipality. Google address components now prioritize administrative level 3, locality, postal town, and first-level sublocality before county/state; neighborhood is only a final fallback. Existing version-1 results are automatically refreshed, so the pictured Jingumae shop becomes `澀谷區（渋谷区）`, Katase becomes `藤澤市（藤沢市）`, Myeongdong becomes `中區（중구）`, and Montmartre groups under Paris. Frontend assets advanced to `20260903.3`; all 100 deployment checks pass.
