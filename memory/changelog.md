...
- Corrected the Google Maps device handoff after an audit found that `window.open(url, "_blank", "noopener")` can return `null` even when the desktop tab opened successfully, causing the App tab to navigate away as well. Desktop now opens a same-origin blank tab, removes its opener, and replaces that tab's location; only a genuinely blocked popup or failed navigation falls back to the App tab. Phones and tablets still navigate in place for native Maps handoff. Unit coverage now models successful, blocked, and failed desktop navigation separately.
