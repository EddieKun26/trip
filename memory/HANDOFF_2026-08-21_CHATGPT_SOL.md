...
所有 Google Maps 入口都已收斂到同一裝置分流，包括地點詳情、機場 map node、交通路線按鈕與地圖上的交通 marker；一般社群／訂票來源連結則仍維持安全的新分頁開啟。測試會阻止交通入口再次直接呼叫 `window.open()`。

同輪也將底部 `總覽／地點／行程／購物` 的 `◇ ● □ ▱` 抽象字元替換為統一 Tabler 語意圖示，保留文字標籤，加入品牌紅選中底座、54px 以上觸控高度、`aria-current="page"`、焦點樣式與 reduced-motion 規則。375×812 瀏覽器檢查確認無水平溢出、四個頁面選中狀態正確且無主控台錯誤。前端資產版本更新為 `20260821.5`。

---

## 8. 2026-08-22 全螢幕地圖與購物地點

- 地點地圖新增 data-toggle-map-fullscreen 入口。全螢幕不是依賴 Safari 支援不完整的原生 Fullscreen API，而是把 App 外框切換成固定滿版工作區；底部導覽與手機外框在此模式隱藏。
- 桌面版左側為 286px 功能欄，包含規劃／當日模式、日期、類型、想去程度、定位、各類數量及符合條件的地點清單。點清單會直接平移並放大目前 Google／Leaflet 地圖。欄位可收合，收合後會要求地圖重新計算尺寸。
- 700px 以下左欄改為疊在地圖上的抽屜；選取地點後自動收合，右上仍保留篩選與離開全圖按鈕。桌面可用 Escape 離開。
- 地點種類新增 shopping／「購物」，可辨識服飾、鞋店、百貨、商場、選物、精品與藥妝等，也能在匯入時手動指定。此類別會進入收藏地點、地圖篩選與加入行程，但與底部分頁中每位成員私有的採買清單完全分開。
- 規劃地圖改用與收藏地點相同的 h1 頁面標題層級。地圖主圖標由 42px 縮至 34px，票數／順序徽章、邊框、陰影及 Leaflet anchor 同步縮小，避免文字與邊框間留白過多。
- 資產版本為 20260822.1；node --check 通過，node --test 為 **92 / 92 通過**。
