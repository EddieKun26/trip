...
- Verify that existing Jingumae, Jinnan, and Yoyogi places regroup under `澀谷區（渋谷区）`, Shinjuku under `新宿區（新宿区）`, and neighborhood-level records elsewhere regroup under their Google-resolved ward/district or municipality in a signed-in production trip; then spot-check one Korean and one Latin-script destination.
