...
- Link-assisted place import supports Instagram posts/Reels, Threads, Agoda, Booking.com, and Airbnb. AI extracts possible place mentions from untrusted public metadata, the allowlisted source URL, or one screenshot; booking-platform links are forced into the lodging category and may use web search when page metadata is unavailable. Google Places remains the source of truth for the saved name, address, category, rating, hours, phone, photos, and coordinates. A user must explicitly select a Google candidate and confirm before saving. The original source URL is retained as a reference, while source images are not copied into shared place data.
