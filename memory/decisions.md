...
- A shopping item's original recognition screenshot remains private Redis data. Product photos are not manually uploaded or copied into the database: AI research identifies cited product pages, the server extracts safe public HTTPS image metadata from those pages, and the item stores only source-linked image URLs plus the member's preferred thumbnail URL.
- AI shopping research is explicit and on demand, never automatic on detail open. It reads the authenticated member's persisted private item server-side, uses OpenAI Responses API web search with structured output and complete source metadata, stores no OpenAI response, and persists the returned annotation and source-linked product-image candidates only through the private Shopping record.
- The 1-5 AI score is labeled as a shopping-reference index based on information completeness, source consistency, clarity of use, and in-store identification value. It is never presented as a medical efficacy, safety, or personalized recommendation score; medicine and supplement guidance remains subordinate to official labeling and clinicians.
