...
- A shopping item's original recognition screenshot and later product photos are separate fields. Each item may keep up to four added product photos within the existing private photo-count and payload ceilings, and the first added product photo is preferred as the list thumbnail.
- AI shopping research is explicit and on demand, never automatic on detail open. It reads the authenticated member's persisted private item server-side, uses OpenAI Responses API web search with structured output, stores no OpenAI response, and persists the returned annotation only through the private Shopping record.
