...
- A shopping item's original recognition screenshot remains private Redis data. Product photos are not manually uploaded or copied into the database: the same OpenAI request that recognizes the screenshot also performs web-grounded research, after which the server extracts one likely front-facing public HTTPS image URL from a cited matching product page. Only that URL is stored with the private item.
- Each new shopping screenshot spends one OpenAI Responses request that combines multilingual visual recognition, web search, structured brand/name/category/benefit output, summary, features, general usage, and cautions. Opening or saving the resulting item does not trigger another AI request; an explicit backfill action remains only for legacy or manually added records.
- Product-source links, recommendation stars, and numeric recommendation indexes are intentionally hidden from the Shopping UI. The detail view presents concise product information with a health-information disclaimer, while source metadata can remain server-side for grounding and image discovery.
