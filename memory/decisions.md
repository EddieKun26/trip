...
- Public Instagram/Threads Open Graph images and video covers are fetched only from allowlisted Meta CDN hosts, with image MIME and size validation, then included in the existing recognition request. A cover is treated as evidence only when it visibly contains a name/address; the system does not scrape private data or infer a business from generic scenery. Full video frames, carousels, and replies require official platform access or user-provided screenshots when the public page does not expose them.
