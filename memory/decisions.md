...
- Shopping import must present up to three product-image candidates before confirmation. `換一批圖片` performs a dedicated web image search for the already-recognized brand and product, excludes candidate IDs seen in earlier rounds, and never reruns image generation.
- Multi-image Shopping imports dispatch all accepted screenshots concurrently, up to eight. Completion and refresh state is tracked per item so an already-recognized card remains interactive while other cards are still running. Browser file pickers cannot enforce a native maximum count, so selections over eight are rejected before compression or API use instead of being silently truncated.
- Selected web product images are converted to bounded JPEG data URLs for reliable iPhone Canvas support. The private Shopping save accepts up to 1.2 MB total selected-product imagery, and the client reconciles each newly added item with the server-sanitized response so stripped images cannot be mistaken for a successful save.
