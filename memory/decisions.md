...
- Shopping screenshot recognition uses server-side multilingual vision AI through Vercel AI Gateway rather than local OCR heuristics. It analyzes one image at a time, preserves original-language brand/product names alongside Traditional Chinese meaning, extracts only image-stated benefits, and returns one main product. Requests require a signed-in trip member, are limited per member/day, and never expose credentials to the browser. Gateway authentication always prioritizes the fresh runtime `x-vercel-oidc-token` header over potentially stale configured keys. Recognition first requests a strict schema, then automatically falls back to JSON mode and prompt-constrained JSON when a provider rejects the stricter format. Every field remains editable and nothing is added until confirmation.
