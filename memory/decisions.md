...
- Shopping screenshot recognition uses server-side multilingual vision AI through Vercel AI Gateway rather than local OCR heuristics. It analyzes one image at a time, preserves original-language brand/product names alongside Traditional Chinese meaning, extracts only image-stated benefits, and returns one main product in a strict schema. Requests require a signed-in trip member, are limited per member/day, and never expose credentials to the browser. Every field remains editable and nothing is added until confirmation.
