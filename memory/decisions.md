...
- A shopping item's original recognition screenshot remains private Redis data. One browser upload request runs structured Responses recognition/web verification and a dedicated GPT Image edit in parallel. The displayed product image is generated from the uploaded reference as a centered, front-facing, product-only image on pure white; public website images are never copied, hotlinked, or stored.
- The dedicated Image Edit endpoint is mandatory rather than relying on a model to optionally invoke an image tool. Opening or saving the resulting item does not trigger another AI request; an explicit repair action remains only for legacy or manually added records.
