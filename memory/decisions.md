...
- A shopping item's original recognition screenshot remains private Redis data. The same OpenAI Responses request performs multilingual recognition, web-grounded product verification, structured annotation, and one high-fidelity image edit based on that screenshot. The displayed product image is generated from the uploaded reference as a centered, front-facing, product-only image on pure white; public website images are never copied, hotlinked, or stored.
- Each new shopping screenshot spends one OpenAI Responses request that combines visual recognition, web search, structured brand/name/category/benefit output, summary, features, general usage, cautions, and one generated product image. Opening or saving the resulting item does not trigger another AI request; an explicit repair action remains only for legacy or manually added records.
