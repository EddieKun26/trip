...
- New screenshot imports use one OpenAI Responses request per image to perform multilingual visual identification, web-grounded product verification, structured output, and a high-fidelity image edit from the uploaded screenshot. The result is one centered, front-facing, product-only image on pure white; the app no longer displays or stores website images and no second request is needed when opening the item.
...
- `node --test tests/*.test.mjs`: 63 tests passing as of 2026-08-12 after replacing external product-photo discovery with screenshot-grounded pure-white image generation, stripping source URLs, and adding legacy image repair.
