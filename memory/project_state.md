...
- 可貼入多個 Google Maps 連結批次新增景點；辨識成功後會啟用確認加入，搜尋型網址會保留地點 query 判斷身分，不會把不同景點誤判為同一收藏。
...
- `node --test tests/*.test.mjs` 共十五項測試，另驗證地區中日文轉換、Google Maps 搜尋連結的不同地點識別、退出按鈕位置、機場路線節點、紅色虛線航班、分享連結預填、地圖分類、單日／所有日期路線、時間自動排序、路線圖標內容與非原生拖曳互動契約。
