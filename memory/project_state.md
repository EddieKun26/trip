...
- Instagram and Threads links can be imported through an authenticated AI-assisted review flow. Public post metadata, optional pasted text, or an optional screenshot is used only to infer place mentions; the user then selects a Google Places match before anything is added, and the original post link is retained on the place.
...
- `node --test tests/*.test.mjs`: 51 tests passing as of 2026-08-10 after adding confirmed social-link place import.
- iPhone 15 Pro browser verification at 393×852 confirms the place-import sheet stays inside the viewport, has no page-level overflow, uses 16px inputs, and confines scrolling to the candidate list.
