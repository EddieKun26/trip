...
- Each new Shopping screenshot now uses one OpenAI Responses request that may include a web-search tool call. Product pages, regional labels, and official sources can differ or disappear; the result is a shopping aid rather than authoritative medical, safety, price, or availability advice.
- The single AI-found product photo remains hosted by its source website. Some sites omit usable image metadata, block hotlinking, change URLs, or return a generic logo; the UI may show no separate product image when a trustworthy cited page cannot provide one, but keeps the private original screenshot.
