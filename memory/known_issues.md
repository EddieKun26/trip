...
- AI shopping annotations also use the separately billed OpenAI API and a web-search tool call. Product pages, regional labels, and official sources can differ or disappear; the result is a shopping aid rather than authoritative medical, safety, price, or availability advice.
- AI-found product photos remain hosted by their source websites. Some sites omit usable image metadata, block hotlinking, change URLs, or return a generic logo; the UI retains a source link and may show no image when a trustworthy page cannot provide one.
