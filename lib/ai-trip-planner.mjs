...
import { minutesOf, validatePlannerPlan } from "./ai-trip-planner-validator.mjs";
import { dayOpeningWindows, formatWindow, intervalWithinWindows, trustedOpeningPeriods } from "./opening-hours.mjs";

export const OPENAI_RESPONSES_URL = "https://api.openai.com/v1/responses";
export const PLANNER_MAX_MODEL_CALLS = 2;
/* Per-model-call timeout, generous against the observed real Luna High tail (max ≈ 17.75s over the
 * Phase 2A.3 runs) so a slow-but-healthy call is never aborted, while a hung connection still ends
 * as this path's own PLANNER_UPSTREAM_FAILED. Both sequential calls plus preprocessing/validation
 * must fit inside the route's declared maxDuration (vercel.json + api/trip.mjs, asserted by a
 * test), which in turn stays below the client's own planning timeout, so the server always answers
 * first with a coded error instead of being killed by the platform. */
export const PLANNER_MODEL_TIMEOUT_MS = 60000;
...
export function plannerTripDays(trip) {
  ...
  for (const current = new Date(start); current <= end && days.length < 61; current.setUTCDate(current.getUTCDate() + 1)) {
    days.push({ dayKey: `${current.getUTCMonth() + 1}/${current.getUTCDate()}`, weekday: WEEKDAYS[current.getUTCDay()], weekdayIndex: current.getUTCDay() });
  }
  return days;
}
...
/* Normalized availability per trip day from the Place's stored structured regular hours:
 * { [dayKey]: [{ startMinute, endMinute }] } on each day's extended local timeline ([] = known
 * closed), or null = hours unknown (absent, unavailable, malformed or not bound to this Place's
 * placeId). The model input and the validator both read this one value. Never fetched here. */
export function plannerOpeningWindows(place, days) {
  const periods = trustedOpeningPeriods(place);
  if (!periods) return null;
  return Object.fromEntries(days.map((day) => [day.dayKey, dayOpeningWindows(periods, day.weekdayIndex, PLANNER_DAY_HORIZON)]));
}

/* Whether a known-hours day can hold at least a minimum-length visit: for an exact start, the
 * interval [exact, exact + minimum) fits one window; otherwise some window fits the minimum at
 * a legal start. Unknown hours never remove anything. */
function openingAllows(candidate, option) {
  const windows = candidate.openingWindows?.[option.dayKey];
  if (!windows) return true;
  if (option.mode === "exact") {
    const start = minutesOf(option.exactTime);
    return start !== null && intervalWithinWindows(windows, start, start + PLANNER_MIN_DURATION);
  }
  return windows.some((window) => window.endMinute - window.startMinute >= PLANNER_MIN_DURATION);
}
...
export function buildPlannerContext({ trip, places, selected }) {
  ...
  for (const { key, place } of eligible) {
    ...
    candidates.push({
      ...
      openingWindows: plannerOpeningWindows(place, days),
    });
  }
 ...
 * Place and none when an existing itinerary item already starts at that time. A required Place's
 * day edge is dropped only when its known opening hours prove that day cannot hold even a
 * minimum-length visit (at the exact time, when one is set); preferred periods never take part.
 * Soft candidates never participate. */
export function checkPlannerFeasibility(context, { exactSlots = true, openingHours = true } = {}) {
  ...
  for (const candidate of required) {
    ...
    for (const option of options) {
      if (openingHours && !openingAllows(candidate, option)) continue;
  ...
  const reason = openingHours && checkPlannerFeasibility(context, { exactSlots, openingHours: false }).feasible ? "OPENING_HOURS_CONFLICT"
    : exactSlots && checkPlannerFeasibility(context, { exactSlots: false, openingHours: false }).feasible ? "EXACT_TIME_CONFLICT" : "CAPACITY_EXCEEDED";
...
export function plannerModelInput(context) {
  return {
    ...
    candidates: context.candidates.map((candidate) => ({
      ...
      ...plannerOpeningHoursInput(candidate, context.days),
    })),
  };
}

/* Compact, trip-date-relevant opening hours for the model: only the days this candidate may use,
 * as local wall-clock window labels ([] = closed that day). Unknown hours are explicit. */
function plannerOpeningHoursInput(candidate, days) {
  if (!candidate.openingWindows) return { hoursUnknown: true };
  const dayKeys = candidate.dateOptions.length ? candidate.dateOptions.map((option) => option.dayKey) : days.map((day) => day.dayKey);
  return { openingHours: Object.fromEntries(dayKeys.map((dayKey) => [dayKey, (candidate.openingWindows[dayKey] || []).map((window) => formatWindow(window, PLANNER_DAY_HORIZON))])) };
}
...
export async function callPlannerModel({ apiKey, model, effort, context, repair = null, fetchImpl = fetch, timeoutMs = PLANNER_MODEL_TIMEOUT_MS }) {
