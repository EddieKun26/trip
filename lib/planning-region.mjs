...
import canonicalCatalog from './canonical-travel-catalog.js';
...
registerArea(JP_NAMED_TRAVEL_AREAS, { key: "shibuya", zh: "澀谷", local: "渋谷", resolver: "JP_TRAVEL_AREA" }, ["神宮前", "Jingumae", "神南", "Jinnan", "富ヶ谷", "富ケ谷", "Tomigaya", "渋谷", "Shibuya", "宇田川町", "Udagawacho", "道玄坂", "Dogenzaka", "円山町", "Maruyamacho"]);
...
registerArea(JP_NAMED_TRAVEL_AREAS, { key: "ebisu", zh: "惠比壽", local: "恵比寿", resolver: "JP_TRAVEL_AREA" }, ["恵比寿", "恵比寿南", "惠比壽", "Ebisu", "Ebisu Minami", "Ebisuminami"]);
registerArea(JP_NAMED_TRAVEL_AREAS, { key: "daikanyama", zh: "代官山", local: "代官山", resolver: "JP_TRAVEL_AREA" }, ["代官山", "代官山町", "猿楽町", "猿樂町", "Daikanyama", "Daikanyamacho", "Sarugakucho"]);
registerArea(JP_NAMED_TRAVEL_AREAS, { key: "shinjuku", zh: "新宿", local: "新宿", resolver: "JP_TRAVEL_AREA" }, ["西新宿", "Nishishinjuku", "歌舞伎町", "Kabukicho", "新宿", "Shinjuku"]);
registerArea(JP_NAMED_TRAVEL_AREAS, { key: "otsuka", zh: "大塚", local: "大塚", resolver: "JP_TRAVEL_AREA" }, ["北大塚", "南大塚", "大塚", "Otsuka"]);
registerArea(JP_NAMED_TRAVEL_AREAS, { key: "ikebukuro", zh: "池袋", local: "池袋", resolver: "JP_TRAVEL_AREA" }, ["西池袋", "東池袋", "南池袋", "Minami Ikebukuro", "Minamiikebukuro", "池袋", "Ikebukuro"]);
registerArea(JP_NAMED_TRAVEL_AREAS, { key: "ueno", zh: "上野", local: "上野", resolver: "JP_TRAVEL_AREA" }, ["上野", "上野公園", "Ueno", "Uenokoen", "Ueno Park"]);
...
registerArea(JP_NAMED_TRAVEL_AREAS, { key: "tokyo-tower", zh: "東京鐵塔", local: "東京タワー", resolver: "JP_TRAVEL_AREA" }, ["東京タワー", "東京鐵塔", "Tokyo Tower"]);
registerArea(JP_NAMED_TRAVEL_AREAS, { key: "shiba-park", zh: "芝公園", local: "芝公園", resolver: "JP_TRAVEL_AREA" }, ["芝公園", "Shiba Park", "Shibakoen"]);
registerArea(JP_NAMED_TRAVEL_AREAS, { key: "harajuku", zh: "原宿", local: "原宿", resolver: "JP_TRAVEL_AREA" }, ["原宿", "Harajuku"]);
registerArea(JP_NAMED_TRAVEL_AREAS, { key: "omotesando", zh: "表參道", local: "表参道", resolver: "JP_TRAVEL_AREA" }, ["Omotesando", "表参道", "表參道"]);
...
function areaResult(area, countryCode) {
  ...
  return {
    ...
    travelAreaResolutionStatus: "resolved", travelAreaResolved: true, travelAreaSource: "automatic", travelAreaResolver: area.resolver, travelAreaResolutionVersion: TRAVEL_AREA_RESOLUTION_VERSION,
...
function resolveJapan(components, countryCode) {
  if (components.some(c => /^(?:恵比寿西|惠比壽西|Ebisunishi|Ebisu[ -]Nishi)$/iu.test(c.longText))) return unresolvedResult(countryCode, "AMBIGUOUS_EBISU_DAIKANYAMA");
...
export function resolveTravelArea({ localizedAddressComponents = [], originalAddressComponents = [], countryCode = "" } = {}) {
  ...
  // Only exact catalog labels on the same highest geographic component level can
  // establish a tie. No aliases, name similarity, romanization or nearby expansion.
  // Ebisunishi alone remains insufficient evidence, as in Phase B.
  for (const type of TYPE_PRIORITY) {
    const components = combined.filter(component => component.types.includes(type));
    if (!components.length) continue;
    const keys = Object.values(canonicalCatalog.catalog).filter(area => area.countryCode === resolvedCountryCode
      && components.some(component => [area.travelAreaLocal, area.travelAreaZh].includes(component.longText))).map(area => area.travelAreaKey);
    if (keys.length >= 2) return { ...PlanningGeography.ambiguousAreaFields(keys), countryCode: resolvedCountryCode };
    break;
  }
