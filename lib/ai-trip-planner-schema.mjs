...
11. 營業時間：候選地點有 openingHours 時，它列出該地點在各日期的營業時段（當地時間）。安排在某天時，整段停留時間 startTime 到 startTime + durationMinutes 必須完全落在當天列出的其中一個時段內（可以剛好在時段結束時離開；不能只讓開始時間落在時段內，也不能跨過兩個時段之間的休息空檔）。某天的時段為空陣列代表當天公休，不可安排在那天。「(+1)」代表隔天凌晨，「全天」代表整天營業。hoursUnknown=true 代表沒有可靠的營業時間資料：不代表 24 小時營業，也不代表公休；此時不受這條規則限制，但請依地點類型安排合理的時間。
