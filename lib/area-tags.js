...
(function (root) {
  ...
  // Verified user-facing Travel Area containment: Canonical Area evidence, never an areaTag
  // suggestion. The catalog's own assessment separates an independent travel circle from a
  // ward/municipality fallback boundary. No network, no runtime geocoding: the caller injects
  // the same pre-generated geometry the map already loads.
  ...
  // Raw locality evidence only: saved address components, then the formatted address. A
  // Canonical Area polygon hit names a planning identity, not a locality, so it is never an
  // areaTag suggestion. A second (catalog) argument from older callers is ignored.
  ...
  function suggestions(place, places, selected = values(place)) {
    ...
    // Address locality evidence only, for the editor, autocomplete, candidate drafts and the
    // tag backfill alike. Containment stays Canonical Area evidence (travelAreaHits).
    const candidates = addressSuggestions(place).map((tag) => preferredLabel(tag, places, context));
  ...
  // Raw evidence is not a canonical identity. The legacy locality rewrites are withdrawn.
  const CONTAINMENT_MIGRATIONS = Object.freeze({});
