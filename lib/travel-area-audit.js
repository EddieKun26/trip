...
(function (root) {
  ...
  function legacyKey(place) {
    if (place?.travelAreaResolutionStatus === "ambiguous") return "";
  ...
  function reclassify(place, catalog) {
    // Shared by server GET/PUT, hydration and deferred legacy normalization.
    // An explicit manual identity wins even when old compatibility labels remain.
    if (place?.travelAreaManuallySet === true || place?.travelAreaSource === 'manual') return place;
