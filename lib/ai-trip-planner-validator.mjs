...
import { formatMinute, formatWindow, intervalWithinWindows } from "./opening-hours.mjs";
...
/* context: { days: [{dayKey}], candidates: [{ref, required, kind, dateOptions, openingWindows}], existingByDay:
...
export function validatePlannerPlan(plan, context) {
  ...
  for (const day of plan.days) {
    ...
    for (const item of day.items) {
      ...
      // Known opening hours are hard for every AI addition (required or soft): the whole visit
      // [start, start + duration) must sit inside one window of that day; [] = closed. Unknown
      // hours (null) are never checked. Existing locked items are not part of this check.
      const openingWindows = candidate.openingWindows?.[day.dayKey];
      const visit = itemWindow(startTime, duration);
      if (openingWindows && visit && !intervalWithinWindows(openingWindows, visit.start, visit.end)) {
        error("OUTSIDE_OPENING_HOURS", { candidateRef: ref, dayKey: day.dayKey, startTime, endTime: formatMinute(visit.end),
          openingWindows: openingWindows.map((window) => formatWindow(window, PLANNER_DAY_HORIZON)) });
      }
...
export function planQualityMetrics(context, validation) {
  ...
  const hoursChecked = [...placed.entries()].flatMap(([ref, item]) => {
    const windows = candidates.get(ref)?.openingWindows?.[item.dayKey];
    const visit = itemWindow(item.startTime, item.durationMinutes);
    return windows && visit ? [{ ok: intervalWithinWindows(windows, visit.start, visit.end) }] : [];
  });
  const legs = [];
  const perDay = [];
  ...
  return {
    ...
    // AI additions whose day has known hours, and how many of them sit inside a window.
    openingHoursChecked: hoursChecked.length,
    openingHoursCompliance: ratio(hoursChecked.filter((item) => item.ok).length, hoursChecked.length),
