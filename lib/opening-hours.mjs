...
export function trustedOpeningPeriods(place) {
  ...
  if (!isPlainObject(record) || record.v !== OPENING_PERIODS_VERSION || record.status !== "known") return null;
  if (!placeId || NON_GOOGLE_ID.test(placeId) || record.placeId !== placeId) return null;
  return sanitizeOpeningPeriods(record.periods);
}
