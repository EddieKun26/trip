...
export async function readThreadsSource(sourceUrl, { lookup, request = httpsRequest, includeMedia = false } = {}) {
  ...
  async function worker() {
    while (cursor < draft.imageCandidates.length) {
      ...
      try {
        ...
        if (includeMedia) candidate.dataUrl = `data:${image.mime};base64,${image.body.toString("base64")}`;
      } catch (error) {
