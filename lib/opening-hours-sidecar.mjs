...
import { resolveStructuredOpeningPeriods } from "./opening-hours.mjs";
...
export async function readOpeningHoursSidecars(places, redisCommand) {
  const ids = [...new Set(places.map(place => String(place?.placeId || "").trim()).filter(Boolean))];
...
export function overlayOpeningHours(trip, sidecars) {
  return { ...trip, places: (trip.places || []).map(place => {
    const effective = resolveStructuredOpeningPeriods(place, sidecars.get(place?.placeId));
